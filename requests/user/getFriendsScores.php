...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {//checking that the request comes from a POST method
    ...
    $id = mysql_real_escape_string($data->user); //Getting the id value
    $page = mysql_real_escape_string($data->page); //Getting the id value
    $sql = "SELECT * FROM users u LEFT JOIN users_has_friends uf on uf.users_b=u.id "
            . " WHERE uf.users_a=$id"
            . " UNION SELECT * FROM users u LEFT JOIN users_has_friends uf on uf.users_a=u.id"
            . " WHERE uf.users_b=$id"
            . " ORDER BY age DESC"; //Query
...
//        $r["password"]=$row["password"];
        ...
        $response = array('status' => $status, 'users' => $users, 'count' => $total, 'pages' => $p, 'sql' => $sql); //sending the flag and the user
