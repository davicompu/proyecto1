...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {//checking that the request comes from a POST method
    ...
    $sql = "SELECT * FROM  users u ORDER BY u.age"; //Query
    $result = $connection->query($sql); //executing query
    $total = $result->num_rows;
    if ($page == 0) {
        if ($total > 10) {
            $sql = "SELECT * FROM  users u ORDER BY u.age DESC LIMIT 10"; //Query
            $result = $connection->query($sql); //executing query
        }
//echo "query: ".$sql;
    } else {
        $page = 10 * ($page - 1);
        $sql = "SELECT * FROM  users u ORDER BY u.age DESC LIMIT $page,10"; //Query
        $result = $connection->query($sql); //executing query
    }
    $users = array();
